...
use soma::kernel::{AwaitOutcome, Kernel, RuntimeError};
...
fn spawn_leaf(kernel: &mut Kernel, process: Ref64) -> Ref64 {
    kernel.create_continuation(process, SEARCH_BRANCH, 0, leaf_bytes(), DEFAULT_MAX_STEPS)
}
...
fn exhausted_step_budget_faults_before_dispatch_and_leaves_no_bin_entry() {
    ...
    let cont = kernel.create_continuation(p, EXPAND_RESUME_1, EXPAND_RESUME_1, bytes, 1);
...
fn blocked_reply_does_not_respawn_children_on_re_entry() {
    ...
    for _ in 0..8 {
        let filler = kernel.create_object(ObjectKind::MessagePayload, vec![0u8; 8]);
        kernel
            .ingest_message(requester, requester, filler, Ref64::NULL)
            .unwrap();
    }
    ...
    kernel
        ...
        .entries
        .pop_front();
    kernel.continuations.get_mut(cont).unwrap().status = ContinuationState::Runnable;
    kernel.scheduler.enqueue(EXPAND_RESUME_2, cont);
    ...
    let mailbox = kernel.mailboxes.get(&requester.slot).unwrap();
    let reply = mailbox.entries.back().unwrap();
    assert_eq!(
        kernel.read_u64_object(reply.payload),
        Some(15),
...
fn every_blocked_sender_is_registered_and_woken_in_order() {
    ...
    let sender_a = kernel.create_process(ProcessMode::Serial);
    let sender_b = kernel.create_process(ProcessMode::Serial);
    ...
    for _ in 0..8 {
        let filler = kernel.create_object(ObjectKind::MessagePayload, vec![0u8; 8]);
        kernel
            .ingest_message(sender_a, receiver, filler, Ref64::NULL)
            .unwrap();
    }

    let payload_a = kernel.create_object(ObjectKind::MessagePayload, vec![1u8; 8]);
    let payload_b = kernel.create_object(ObjectKind::MessagePayload, vec![2u8; 8]);
    assert_eq!(
    ...
    assert_eq!(
        kernel.mailboxes.get(&receiver.slot).unwrap().full_waiters.len(),
        2
    );
    ...
    kernel.receive_message(cont_r).unwrap().unwrap();

    let mailbox = kernel.mailboxes.get(&receiver.slot).unwrap();
    assert_eq!(mailbox.full_waiters.len(), 1);
    assert_eq!(
        mailbox.full_waiters.front().copied(),
        Some(cont_b),
...
fn awaiting_a_resolved_future_yields_instead_of_parking() {
    ...
    let resolved = kernel.create_future();
    let value = kernel.create_object(ObjectKind::FutureValue, 42u64.to_le_bytes().to_vec());
    kernel.resolve_future(resolved, value).unwrap();

    // Registering here would park forever: resolve_future drains its waiter
    // list exactly once, and that already happened.
    assert_eq!(
        kernel.await_future(cont, resolved, EXPAND_RESUME_1).unwrap(),
    ...
    let pending = kernel.create_future();
    assert_eq!(
        kernel.await_future(cont, pending, EXPAND_RESUME_1).unwrap(),
    ...
    let value2 = kernel.create_object(ObjectKind::FutureValue, 1u64.to_le_bytes().to_vec());
    kernel.resolve_future(pending, value2).unwrap();
    assert_eq!(
        kernel.continuation_state(cont).unwrap(),
        ContinuationState::Runnable
    );
...
fn futures_are_single_assignment() {
    ...
    let first = kernel.create_object(ObjectKind::FutureValue, 1u64.to_le_bytes().to_vec());
    kernel.resolve_future(future, first).unwrap();

    let second = kernel.create_object(ObjectKind::FutureValue, 2u64.to_le_bytes().to_vec());
    assert_eq!(
        kernel.resolve_future(future, second),
...
fn freezing_is_idempotent_and_makes_an_object_untransferable() {
    let mut kernel = Kernel::new();
    let owner = kernel.create_process(ProcessMode::Serial);
    let other = kernel.create_process(ProcessMode::Serial);
    let object = kernel.create_object(ObjectKind::MessagePayload, vec![1, 2, 3]);

    assert_eq!(
        ownership_state(&kernel, object).unwrap(),
        OwnershipState::UniqueMutable
    );
    transfer_unique(&mut kernel, object, other).unwrap();

    let version = freeze(&mut kernel, object).unwrap();
    assert_eq!(
        ownership_state(&kernel, object).unwrap(),
        OwnershipState::FrozenShared
    );
    assert_eq!(
        freeze(&mut kernel, object).unwrap(),
        version,
        "freezing an already-frozen object does not bump the version again"
    );
    assert!(
        transfer_unique(&mut kernel, object, owner).is_err(),
