...
fn main() {
    ...
    eprintln!("resident_dynamic_state_rich_fair: lanes={LANES} steps={STEPS} work={WORK} batches={BATCHES} samples={SAMPLES} warmups={WARMUPS} compile_us={compile_us}");
    eprintln!("fair AB/BA state-rich grouped-one-submit versus level-sync-16-submit; equal 25 ms pre-pair cooldown; all frames and canonical traces exact");
    println!(
    ...
    for batch in 0..BATCHES {
        ...
        for sample in 0..SAMPLES {
            ...
            println!(
                ...
                if grouped_first { "grouped" } else { "level_sync" }
            );
        }
    }
}
...
fn main() {
    eprintln!("resident_dynamic_state_rich_fair requires macOS and --features metal");
}
